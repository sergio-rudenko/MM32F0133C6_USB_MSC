...
extern "C" {
  ...
  {
    if (rb) {
      volatile int readIndex = rb->readIndex; /* NOTE! */
      ...
      while (offset--)
      {
        ...
        else if (rb->writeIndex == readIndex)
        {
          rb->overrun = true;
        }
      }
  ...
  size_t ring_buffer_available(rBuffer_t *rb)
  {
    ...
    if (rb) {
      volatile int writeIndex = rb->writeIndex;	 /* NOTE! */
      volatile int readIndex = rb->readIndex;	 /* NOTE! */

      if (writeIndex > readIndex)
      {
        bytes = writeIndex - readIndex;
      }
      else if (writeIndex < readIndex)
      {
        bytes = rb->size - readIndex + writeIndex;
      }
      else if (rb->overrun)
      {
        bytes = rb->size;
